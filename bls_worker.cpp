...
struct Aggregator {
    ...
    void Start()
    {
		...
		//std::cout<<"finally coming here check"<<std::endl; //VRC
        ...
        IncWait();
	// std::cout<<"batch count is "<<batchCount<<std::endl; //VRC
        for (size_t i = 0; i < batchCount; i++) {
            size_t start = i * batchSize;
            size_t count = std::min(batchSize, inputVec->size() - start);
	    std::cout<<i<<"  --> i"<<std::endl; //VRC OP
    ...
    {
        IncWait();
        PushWork(std::bind(&Aggregator::SyncAggregateAndPushAggQueue, this, vec, start, count, del)); //VRC The function SyncAggregateAndPushAggQueue( ) is called 
    }
    ...
    void Start()
    {
        ...
        for (size_t i = 0; i < vecSize; i++) {
            ...
            for (size_t j = 0; j < count; j++) {
                tmp[j] = &(*vecs[start + j])[i]; // what it does ? - doubt VRC
            }
	    ...
	    std::cout<<"second agg Start()"<<std::endl; //VRC OP
            agg->Start();
        }
    }
...
struct ContributionVerifier {
    ...
    void Start()
    {
        if (!aggregated) {  // aggregated = False
            ...
            batchSize = vvecs.size(); // batchSize = 10
	...
	std::cout<<typeid(batchStates).name()<<'\n'; 
        for (size_t i = 0; i < batchCount; i++) {
...
{
    // Id is the member id | vvces are the verification vecs of all the members | skShares = secret ket shares obtained for that member | 
    if (!forId.IsValid() || !VerifyVerificationVectors(vvecs)) {
        std::vector<bool> result;
        result.assign(vvecs.size(), false);
        doneCallback(result);
        return;
    } // all invalid ids and vvces are removed from the next steps of verification
//    std::cout<<"all valid ids and vvces, now entering ContributionVerifier"<<'\n'; //VRC OP
