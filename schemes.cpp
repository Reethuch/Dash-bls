...
G2Element CoreMPL::Aggregate(const vector<G2Element> &signatures)
{
    ...
    bool flag = true;
    std::out<<"                     C1                         "<<std::flush;
    for (const G2Element& signature : signatures) {
        aggregated += signature;
	std::cout<<"aggregating here only"<<std::flush; //VRC testing here
    }
    return aggregated;
}
...
G2Element CoreMPL::New_Aggregate(const vector<G2Element> &signatures)
{
    G2Element aggregated;
    std::out<<"C1";
    while (signatures.size()>1) {
        G2Element temp;
        for(i=0; i<signatures.size(); i+2){
	    temp.push_back(signatures[i]*signatures[i+1])
	}
	aggregated = temp
	cout<<"aggregating here only"<<endl; //VRC testing here
    }
    return aggregated;
}
//VRC DONE
