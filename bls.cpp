...
{    
//    std::cout<<"scheme bool "<<std::flush; //VRC
...
{
    // vrc std::cout<<"MakeNewKey() call"<<std::endl; //VRC OP
...
{
//	std::cout<<"CHECK 1 VRC"<<std::flush;
    assert(IsValid() && o.IsValid());
    impl = Scheme(fLegacy)->Aggregate({impl, o.impl});
    cachedHash.SetNull();
}
...
CBLSSignature CBLSSignature::AggregateInsecure(const std::vector<CBLSSignature>& sigs, const bool fLegacy)
{
    ...
    bool flag = false; //VRC
 //   std::cout<<" CHECK2VRC "<<std::flush; //RUNNING
    CBLSSignature ret;
//    std::cout<<typeid(v).name()<<std::flush; //VRC
    ret.impl = Scheme(fLegacy)->Aggregate(v); 
   /* std::cout<<ret.impl<<std::flush;
    std::cout<<"checking for typeid of ret:: " <<std::flush; //VRC  
    std::cout<<typeid(ret.impl).name()<<std::flush; //VRC`
    std::cout<<flag<<std::flush;
    std::cout<<"                 "<<std::endl;   // Which Aggregate fnc??? */
    ret.fValid = true;
    ret.cachedHash.SetNull();
    return ret;
}
