...
{   
    std::cout<<"check check G"<<std::endl; //VRC
...
// this function is raising to the power of generator G2 | g2^(private key) or g2 * (private key)
{
    if (!fG2CacheValid) {
        CheckKeyData();
...
PrivateKey PrivateKey::Aggregate(std::vector<PrivateKey> const &privateKeys)
{
    ...
    PrivateKey ret;
    assert(ret.IsZero());
    std::cout<<"ok ok ok"<<std::flush; //VRC
